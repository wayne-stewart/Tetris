...
fn handle_request(client: &mut ClientConnection) -> std::io::Result<()> {
    ...
    //println!("verb: {}, path: {}, http: {}", verb, path, http_version);
...
fn send_file(http_context: &mut HttpContext, mut file: File) -> std::io::Result<()> {
    ...
    let content_type = match http_context.path.split(".").last().unwrap() {
        "js" => "text/javascript",
        "css" => "text/css",
        "html" => "text/html",
        _ => "",
    };
    if content_type.len() == 0 { send_400(http_context, "file extension not supported!")?; return Ok(()); }
